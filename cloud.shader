Shader "Shaders/Skybox/CloudLayer"
{
	Properties
	{
		[Header(Cloud)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half cloudGradient = pow(saturate((x - 0.5) * _CloudContrast + 0.5), (-0.3) / log(_Bias) );
				fixed3 cloudColor = lerp(_CloudColor.rgb, _CloudDarkColor.rgb, cloudGradient);
